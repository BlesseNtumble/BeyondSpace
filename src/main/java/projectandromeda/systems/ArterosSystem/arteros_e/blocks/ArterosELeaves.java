...
public class ArterosELeaves extends BlockLeaves {

	protected boolean leavesFancy;
	int[] surroundings;

	public ArterosELeaves(String name) {
		super();
		...
		if (!worldIn.isRemote) {
			if (((Boolean) state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean) state.getValue(DECAYABLE)).booleanValue()) {
				...
				if (!worldIn.isAreaLoaded(pos, 1))
					return; // Forge: prevent decaying leaves from updating
							// neighbors and loading unloaded chunks
				if (worldIn.isAreaLoaded(pos, 6)) // Forge: extend range from 5
													// to 6 to account for
													// neighbor checks in
	...
	public List<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
		...
		list.add(new ItemStack(this.getItemDropped(world.getBlockState(pos), new Random(), fortune)));
		return list;
	}
}