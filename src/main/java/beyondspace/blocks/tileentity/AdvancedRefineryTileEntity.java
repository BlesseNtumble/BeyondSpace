...
public class AdvancedRefineryTileEntity extends TileBaseElectricBlockWithInventory implements ISidedInventory, IFluidHandler
{
	...
	public AdvancedRefineryTileEntity()
	{
		this.storage.setMaxExtract(ConfigManagerCore.hardMode ? 100 : 70);
	}
	...
	public void updateEntity()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			if (this.canProcess() && this.hasEnoughEnergyToRun)
			{
				...
				{
					this.processTicks = AdvancedRefineryTileEntity.PROCESS_TIME_REQUIRED;
				}
				else
				{
					if (--this.processTicks <= 0)
					{
						this.smeltItem();
						this.processTicks = this.canProcess() ? AdvancedRefineryTileEntity.PROCESS_TIME_REQUIRED : 0;
					}
				}
			}
			else
	...
	public void smeltItem()
	{
		if (this.canProcess())
		{
			...
			final int amountToDrain = Math.min(Math.min(oilAmount, fuelSpace), AdvancedRefineryTileEntity.OUTPUT_PER_SECOND);
	...
	{
		return GCCoreUtil.translate("tile.advancedRefinery.name");
	}
