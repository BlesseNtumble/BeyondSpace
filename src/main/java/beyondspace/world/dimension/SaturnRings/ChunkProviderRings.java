...
import beyondspace.world.generation.structures.WorldGenTreasureTrees;
...
public class ChunkProviderRings extends ChunkProviderGenerate
{
    final Block ASTEROID_STONE = Blocks.snow;
    final byte ASTEROID_STONE_META_0 = 0;
    final Block ASTEROID_STONE_1 = AsteroidBlocks.blockDenseIce;
    final byte ASTEROID_STONE_META_1 = 0;
    final Block ASTEROID_STONE_2 = Blocks.packed_ice;
    ...
    //chest
    final Block TREASURE_CHEST = Blocks.chest;
    final byte TREASURE_CHEST_META = 0;
    ...
    private static final int ASTEROID_CHANCE = 180; //About 1 / n chance per XZ pair
    ...
    private static final int CHEST_CHANCE = 2;
    ...
    public ChunkProviderRings(World par1World, long par2, boolean par4)
    {
        ...
        this.asteroidSkewX.amplitude = ChunkProviderRings.MAX_ASTEROID_SKEW;
        this.asteroidSkewX.frequencyX = 0.005F;

        this.asteroidSkewY = new Gradient(this.rand.nextLong(), 1, 1);
        this.asteroidSkewY.amplitude = ChunkProviderRings.MAX_ASTEROID_SKEW;
        this.asteroidSkewY.frequencyY = 0.005F;

        this.asteroidSkewZ = new Gradient(this.rand.nextLong(), 1, 1);
        this.asteroidSkewZ.amplitude = ChunkProviderRings.MAX_ASTEROID_SKEW;
        this.asteroidSkewZ.frequencyZ = 0.005F;

        this.coreHandler = new SpecialAsteroidBlockHandler();
        this.coreHandler.addBlock(new SpecialAsteroidBlock(this.ASTEROID_STONE, this.ASTEROID_STONE_META_0, 5, .3));
        this.coreHandler.addBlock(new SpecialAsteroidBlock(this.ASTEROID_STONE_1, this.ASTEROID_STONE_META_1, 7, .3));
        this.coreHandler.addBlock(new SpecialAsteroidBlock(this.ASTEROID_STONE_2, this.ASTEROID_STONE_META_2, 11, .25));
        this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.blockBasic, (byte) 4, 5, .2)); //Ilmenite
        this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.blockBasic, (byte) 5, 4, .15));//Iron
        this.coreHandler.addBlock(new SpecialAsteroidBlock(AsteroidBlocks.blockBasic, (byte) 3, 3, .2));//Aluminum
        ...
        this.shellHandler.addBlock(new SpecialAsteroidBlock(this.ASTEROID_STONE_1, this.ASTEROID_STONE_META_1, 3, .15));
        this.shellHandler.addBlock(new SpecialAsteroidBlock(this.ASTEROID_STONE_2, this.ASTEROID_STONE_META_2, 1, .15));
    ...
    public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray, boolean flagDataOnly)
    {
        ...
        final int asteroidChance = ChunkProviderRings.ASTEROID_CHANCE;
        final int rangeY = ChunkProviderRings.MAX_ASTEROID_Y - ChunkProviderRings.MIN_ASTEROID_Y;
        final int rangeSize = ChunkProviderRings.MAX_ASTEROID_RADIUS - ChunkProviderRings.MIN_ASTEROID_RADIUS;
        ...
        {
            int minX = i * 16;
            int maxX = minX + ChunkProviderRings.CHUNK_SIZE_X;
            for (int k = chunkZ - 3; k < chunkZ + 3; k++)
            {
                int minZ = k * 16;
                int maxZ = minZ + ChunkProviderRings.CHUNK_SIZE_Z;
                ...
                for (int x = minX; x < maxX; x+=2)
                {
                    for (int z = minZ; z < maxZ; z+=2)
                    {
                        ...
                        if (this.randFromPointPos(x, z) < (this.asteroidDensity.getNoise(x, z) + .4) / asteroidChance)
                        {
                            ...
                            int y = random.nextInt(rangeY) + ChunkProviderRings.MIN_ASTEROID_Y;
                            int size = random.nextInt(rangeSize) + ChunkProviderRings.MIN_ASTEROID_RADIUS;
    ...
    private void generateAsteroid(Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, Block[] blockArray, byte[] metaArray, boolean flagDataOnly)
    {
        ...
        if (rand.nextInt(ChunkProviderRings.ASTEROID_SHELL_CHANCE) == 0)
        {
            shell = this.shellHandler.getBlock(rand, size);
        }

        boolean isHollow = false;
        final float hollowSize = rand.nextFloat() * (ChunkProviderRings.MAX_HOLLOW_SIZE - ChunkProviderRings.MIN_HOLLOW_SIZE) + ChunkProviderRings.MIN_HOLLOW_SIZE;
        if (rand.nextInt(ChunkProviderRings.HOLLOW_CHANCE) == 0 && size >= ChunkProviderRings.MIN_RADIUS_FOR_HOLLOW)
        {
            isHollow = true;
        ...
        ((WorldProviderSaturnRings) this.worldObj.provider).addAsteroid(asteroidX, asteroidY, asteroidZ, size, isHollow ? -1 : core.index);

        final int xMin = this.clamp(Math.max(chunkX, asteroidX - size - ChunkProviderRings.MAX_ASTEROID_SKEW - 2) - chunkX, 0, 16);
        final int zMin = this.clamp(Math.max(chunkZ, asteroidZ - size - ChunkProviderRings.MAX_ASTEROID_SKEW - 2) - chunkZ, 0, 16);
        final int yMin = asteroidY - size - ChunkProviderRings.MAX_ASTEROID_SKEW - 2;
        final int yMax = asteroidY + size + ChunkProviderRings.MAX_ASTEROID_SKEW + 2;
        final int xMax = this.clamp(Math.min(chunkX + 16, asteroidX + size + ChunkProviderRings.MAX_ASTEROID_SKEW + 2) - chunkX, 0, 16);
        final int zMax = this.clamp(Math.min(chunkZ + 16, asteroidZ + size + ChunkProviderRings.MAX_ASTEROID_SKEW + 2) - chunkZ, 0, 16);
        ...
        final float noiseOffsetX = this.randFromPoint(asteroidX, asteroidY, asteroidZ) * ChunkProviderRings.NOISE_OFFSET_SIZE + chunkX;
        final float noiseOffsetY = this.randFromPoint(asteroidX * 7, asteroidY * 11, asteroidZ * 13) * ChunkProviderRings.NOISE_OFFSET_SIZE;
        final float noiseOffsetZ = this.randFromPoint(asteroidX * 17, asteroidY * 23, asteroidZ * 29) * ChunkProviderRings.NOISE_OFFSET_SIZE + chunkZ;
        ...
        for (int x = xMax - 1; x >= xMin; x--)
        {
            ...
            int indexBaseX = x * ChunkProviderRings.CHUNK_SIZE_Y << 4;
            float xx = x + chunkX;
            ...
            for (int z = zMin; z < zMax; z++)
            {
                ...
                int indexBase = indexBaseX | z * ChunkProviderRings.CHUNK_SIZE_Y;
                float zz = z + chunkZ;
                    ...
                    for (int y = yMin; y < yMax; y++)
                    {
                        ...
                        if (distance <= 1)
                        {
                            ...
                            if (Blocks.air == blockArray[indexAbove] && (blockArray[index] == ASTEROID_STONE || blockArray[index] == GRASS))
                            {
                               ...
                               // if(this.rand.nextInt(CHEST_CHANCE) == 0)
                               // {
                               // 	blockArray[indexAbove] = this.TREASURE_CHEST;
                               // 	metaArray[indexAbove] = this.TREASURE_CHEST_META;
                               // }
                            }
                        }
                    }
...
//        long time2 = System.nanoTime();
        ...
        {
            var5[var6] = (byte) BiomeGenBaseSaturnRings.saturnRings.biomeID;
        }
...
//        }
    ...
    {
        return x * ChunkProviderRings.CHUNK_SIZE_Y * 16 | z * ChunkProviderRings.CHUNK_SIZE_Y | y;
    }
    ...
    public void populate(IChunkProvider par1IChunkProvider, int chunkX, int chunkZ)
    {
        int x = chunkX << 4;
        int z = chunkZ << 4;
        if (!ChunkProviderRings.chunksDone.add(new BlockVec3(x, 0, z)))
        	return;
    	...
    	if (this.rand.nextBoolean())
        {
            ...
            double numOfBlocks = this.clamp(this.randFromPoint(chunkX, chunkZ), .4, 1) * ChunkProviderRings.MAX_BLOCKS_PER_CHUNK * density + ChunkProviderRings.MIN_BLOCKS_PER_CHUNK;
            int y0 = this.rand.nextInt(2);
        	Block block;
        	int meta;
        	int yRange = ChunkProviderRings.MAX_ASTEROID_Y - ChunkProviderRings.MIN_ASTEROID_Y;

        	for (int i = 0; i < numOfBlocks; i++)
            {
                int y = this.rand.nextInt(yRange) + ChunkProviderRings.MIN_ASTEROID_Y;

                //50:50 chance vertically as well
                if (y0 == (y / 16) % 2)
                {
                    int px = x + this.rand.nextInt(ChunkProviderRings.CHUNK_SIZE_X);
                    int pz = z + this.rand.nextInt(ChunkProviderRings.CHUNK_SIZE_Z);
        ...
        if (!this.largeAsteroids.isEmpty())
        {
            for(AsteroidData asteroidIndex : this.largeAsteroids)
            {
                ...
                if(ConfigManagerCore.challengeMode || rand.nextInt(ChunkProviderRings.TREE_CHANCE) == 0)
                {
                	int treeType = rand.nextInt(3);
                	if (treeType == 1) treeType = 0;
                	WorldGenTreasureTrees wg = new WorldGenTreasureTrees(false, 2, 0, 0, Blocks.chest, 0, false);
                ...
                if(!treesdone || rand.nextInt(ChunkProviderRings.TALL_GRASS_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if(rand.nextInt(ChunkProviderRings.FLOWER_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if(rand.nextInt(ChunkProviderRings.LAVA_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if(rand.nextInt(ChunkProviderRings.WATER_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
    ...
    {
        for (int x = x0; x < x0 + ChunkProviderRings.CHUNK_SIZE_X; x += 2) {
            for (int z = z0; z < z0 + ChunkProviderRings.CHUNK_SIZE_Z; z += 2) {
                if ((Math.abs(this.randFromPoint(x, z)) < (this.asteroidDensity.getNoise(x, z) + .4) / ChunkProviderRings.ASTEROID_CHANCE))
                {
                    return true;
                }
            }
