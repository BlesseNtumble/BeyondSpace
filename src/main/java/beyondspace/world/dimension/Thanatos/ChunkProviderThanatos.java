...
public class ChunkProviderThanatos extends ChunkProviderGenerate implements IChunkProvider {
	final Block topBlockID = RegistrationsList.antimatter;
    final byte topBlockMeta = 0;
    final Block fillBlockID = RegistrationsList.antimatter;
    final byte fillBlockMeta = 0;
    final Block lowerBlockID = RegistrationsList.antimatter;
    ...
    private BiomeGenBase[] biomesForGeneration = { BiomeGenBaseThanatos.thanatos };
    ...
    public ChunkProviderThanatos(World par1World, long par2, boolean par4) {
    ...
    public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray) {
        ...
        for (int x = 0; x < ChunkProviderThanatos.CHUNK_SIZE_X; x++) {
            for (int z = 0; z < ChunkProviderThanatos.CHUNK_SIZE_Z; z++) {
				...
				for (int y = 0; y < ChunkProviderThanatos.CHUNK_SIZE_Y; y++) {
					if (y < ChunkProviderThanatos.MID_HEIGHT + yDev) {
    ...
    public void createCraters(int chunkX, int chunkZ, Block[] chunkArray, byte[] metaArray) {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
                for (int x = 0; x < ChunkProviderThanatos.CHUNK_SIZE_X; x++) {
                    for (int z = 0; z < ChunkProviderThanatos.CHUNK_SIZE_Z; z++) {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderThanatos.CHUNK_SIZE_X + x, cz * ChunkProviderThanatos.CHUNK_SIZE_Z + z) / ChunkProviderThanatos.CRATER_PROB) {
