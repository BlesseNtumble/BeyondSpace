...
public class WorldProviderNeptune extends WorldProviderAdvancedSpace implements IGalacticraftWorldProvider, IExitHeight, ISolarLevel, ITeleportType, ILightning, IAdvancedSpace {

	public void registerWorldChunkManager() {
		this.dimensionId = BSConfig.neptune;
		this.worldChunkMgr = new WorldChunkManagerNeptune();
	}
	
	public IChunkProvider createChunkGenerator() {
		return new ChunkProviderNeptune(this.worldObj, this.worldObj.getSeed(), "2;" + Block.getIdFromBlock(Blocks.bedrock) + ",45x" + Block.getIdFromBlock(Blocks.stone) + ",25x" + Block.getIdFromBlock(RegistrationsList.silicate) + ",30x" + Block.getIdFromBlock(RegistrationsList.FeNiMetal) + ",5x" + Block.getIdFromBlock(RegistrationsList.diamondLiquid) + ",20x" + Block.getIdFromBlock(Blocks.packed_ice) + ",45x" + Block.getIdFromBlock(RegistrationsList.H2ONH3CH4Ice) + ",40x" + Block.getIdFromBlock(RegistrationsList.H2ONH3CH4Liquid) + ";1");
	}
	...
	public Vec3 getFogColor(float f, float ff) {
		return Vec3.createVectorHelper((double)4/255, (double)28/255, (double)183/255);
	}
	...
	public Vec3 getSkyColor(Entity e, float f) {
		return Vec3.createVectorHelper((double)68/255, (double)92/255, (double)247/255);
	}
	...
	public void onSpaceDimensionChanged(World world, EntityPlayerMP player, boolean ridingAutoRocket) {
		if ((player != null) && (GCPlayerStats.get(player).teleportCooldown <= 0)) {
			...
			if (!world.isRemote) {
				...
				player.addStat(RegistrationsList.neptune, 1);
			}
