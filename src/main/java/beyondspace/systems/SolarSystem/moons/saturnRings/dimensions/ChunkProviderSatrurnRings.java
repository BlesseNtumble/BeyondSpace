...
public class ChunkProviderSatrurnRings extends ChunkProviderBase
{
    ...
    private static final int MAX_ASTEROID_RADIUS = 10;
    ...
    private static final int MAX_ASTEROID_Y = ChunkProviderSatrurnRings.CHUNK_SIZE_Y - 48;
    ...
    public ChunkProviderSatrurnRings(World par1World, long par2, boolean par4)
    {
        ...
        this.asteroidSkewX.amplitude = ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW;
        this.asteroidSkewX.frequencyX = 0.005F;

        this.asteroidSkewY = new Gradient(this.rand.nextLong(), 1, 1);
        this.asteroidSkewY.amplitude = ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW;
        this.asteroidSkewY.frequencyY = 0.005F;

        this.asteroidSkewZ = new Gradient(this.rand.nextLong(), 1, 1);
        this.asteroidSkewZ.amplitude = ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW;
    ...
    public void generateTerrain(int chunkX, int chunkZ, ChunkPrimer primer, boolean flagDataOnly)
    {
        ...
        final int asteroidChance = ChunkProviderSatrurnRings.ASTEROID_CHANCE;
        final int rangeY = ChunkProviderSatrurnRings.MAX_ASTEROID_Y - ChunkProviderSatrurnRings.MIN_ASTEROID_Y;
        final int rangeSize = ChunkProviderSatrurnRings.MAX_ASTEROID_RADIUS - ChunkProviderSatrurnRings.MIN_ASTEROID_RADIUS;
        ...
        {
            int minX = i * 16;
            int maxX = minX + ChunkProviderSatrurnRings.CHUNK_SIZE_X;
            for (int k = chunkZ - 3; k < chunkZ + 3; k++)
            {
                int minZ = k * 16;
                int maxZ = minZ + ChunkProviderSatrurnRings.CHUNK_SIZE_Z;
                ...
                for (int x = minX; x < maxX; x += 2)
                {
                    for (int z = minZ; z < maxZ; z += 2)
                    {
                        ...
                        if (this.randFromPointPos(x, z) < (this.asteroidDensity.getNoise(x, z) + .4) / asteroidChance)
                        {
                            ...
                            int y = random.nextInt(rangeY) + ChunkProviderSatrurnRings.MIN_ASTEROID_Y;
                            int size = random.nextInt(rangeSize) + ChunkProviderSatrurnRings.MIN_ASTEROID_RADIUS;
    ...
    private void generateAsteroid(Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, ChunkPrimer primer, boolean flagDataOnly)
    {
        ...
        if (rand.nextInt(ChunkProviderSatrurnRings.ASTEROID_SHELL_CHANCE) == 0)
        {
            shell = this.shellHandler.getBlock(rand, size);
        }

        boolean isHollow = false;
        final float hollowSize = rand.nextFloat() * (ChunkProviderSatrurnRings.MAX_HOLLOW_SIZE - ChunkProviderSatrurnRings.MIN_HOLLOW_SIZE) + ChunkProviderSatrurnRings.MIN_HOLLOW_SIZE;
        if (rand.nextInt(ChunkProviderSatrurnRings.HOLLOW_CHANCE) == 0 && size >= ChunkProviderSatrurnRings.MIN_RADIUS_FOR_HOLLOW)
        {
            isHollow = true;
        ...
        ((WorldProviderSaturnRings) this.world.provider).addAsteroid(asteroidX, asteroidY, asteroidZ, size, isHollow ? -1 : core.index);

        final int xMin = this.clamp(Math.max(chunkX, asteroidX - size - ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW - 2) - chunkX, 0, 16);
        final int zMin = this.clamp(Math.max(chunkZ, asteroidZ - size - ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW - 2) - chunkZ, 0, 16);
        final int yMin = asteroidY - size - ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW - 2;
        final int yMax = asteroidY + size + ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW + 2;
        final int xMax = this.clamp(Math.min(chunkX + 16, asteroidX + size + ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW + 2) - chunkX, 0, 16);
        final int zMax = this.clamp(Math.min(chunkZ + 16, asteroidZ + size + ChunkProviderSatrurnRings.MAX_ASTEROID_SKEW + 2) - chunkZ, 0, 16);
        ...
        final float noiseOffsetX = this.randFromPoint(asteroidX, asteroidY, asteroidZ) * ChunkProviderSatrurnRings.NOISE_OFFSET_SIZE + chunkX;
        final float noiseOffsetY = this.randFromPoint(asteroidX * 7, asteroidY * 11, asteroidZ * 13) * ChunkProviderSatrurnRings.NOISE_OFFSET_SIZE;
        final float noiseOffsetZ = this.randFromPoint(asteroidX * 17, asteroidY * 23, asteroidZ * 29) * ChunkProviderSatrurnRings.NOISE_OFFSET_SIZE + chunkZ;
        ...
        for (int x = xMax - 1; x >= xMin; x--)
        {
            ...
            int indexBaseX = x * ChunkProviderSatrurnRings.CHUNK_SIZE_Y << 4;
            float xx = x + chunkX;
            ...
            for (int z = zMin; z < zMax; z++)
            {
                ...
                int indexBase = indexBaseX | z * ChunkProviderSatrurnRings.CHUNK_SIZE_Y;
                float zz = z + chunkZ;
...
//        }
    ...
    {
        return x * ChunkProviderSatrurnRings.CHUNK_SIZE_Y * 16 | z * ChunkProviderSatrurnRings.CHUNK_SIZE_Y | y;
    }
    ...
    public void populate(int chunkX, int chunkZ)
    {
        int x = chunkX << 4;
        int z = chunkZ << 4;
        if (!ChunkProviderSatrurnRings.chunksDone.add(new BlockVec3(x, 0, z)))
        {
            return;
        }
...
//        BlockFalling.fallInstantly = false;
        ...
        if (this.rand.nextBoolean())
        {
            ...
            double numOfBlocks = this.clamp(this.randFromPoint(chunkX, chunkZ), .4, 1) * ChunkProviderSatrurnRings.MAX_BLOCKS_PER_CHUNK * density + ChunkProviderSatrurnRings.MIN_BLOCKS_PER_CHUNK;
            int y0 = this.rand.nextInt(2);
            Block block;
            int meta;
            int yRange = ChunkProviderSatrurnRings.MAX_ASTEROID_Y - ChunkProviderSatrurnRings.MIN_ASTEROID_Y;
            x += 4;
            z += 4;

            for (int i = 0; i < numOfBlocks; i++)
            {
                int y = this.rand.nextInt(yRange) + ChunkProviderSatrurnRings.MIN_ASTEROID_Y;

                //50:50 chance vertically as well
                if (y0 == (y / 16) % 2)
                {
                    int px = x + this.rand.nextInt(ChunkProviderSatrurnRings.CHUNK_SIZE_X) + 8;
                    int pz = z + this.rand.nextInt(ChunkProviderSatrurnRings.CHUNK_SIZE_Z) + 8;
...
/*
        ...
        if (!this.largeAsteroids.isEmpty())
        {
            for (AsteroidData asteroidIndex : new ArrayList<AsteroidData>(this.largeAsteroids))
            {
                ...
                if (rand.nextInt(ChunkProviderSatrurnRings.TREE_CHANCE) == 0)
                {
                ...
                if (!treesdone || rand.nextInt(ChunkProviderSatrurnRings.TALL_GRASS_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if (rand.nextInt(ChunkProviderSatrurnRings.FLOWER_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if (rand.nextInt(ChunkProviderSatrurnRings.LAVA_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
                ...
                if (rand.nextInt(ChunkProviderSatrurnRings.WATER_CHANCE) == 0)
                {
                    int i = rand.nextInt(16) + x + 8;
                    int k = rand.nextInt(16) + z + 8;
...
//        boolean flagXZDChunk = w.getChunkProvider().chunkExists(cx - 1, cz - 1);
                ...
                for (int z = a.zMinArray; z < a.zMax; z++)
                {
                    for (int y = yMin; y < yMax; y++)
                    {
                        if (chunk.getBlockState(x - 1, y, z).getBlock() instanceof BlockAir && !(chunk.getBlockState(x, y, z).getBlock() instanceof BlockAir))
                        {
                            ...
                            if (count > 8) count = 8;
                            chunk.setBlockState(new BlockPos(x - 1, y, z), GCBlocks.brightAir.getStateFromMeta(10 - count));
    ...
    {
        for (int x = x0; x < x0 + ChunkProviderSatrurnRings.CHUNK_SIZE_X; x += 2)
        {
            for (int z = z0; z < z0 + ChunkProviderSatrurnRings.CHUNK_SIZE_Z; z += 2)
            {
                if ((Math.abs(this.randFromPoint(x, z)) < (this.asteroidDensity.getNoise(x, z) + .4) / ChunkProviderSatrurnRings.ASTEROID_CHANCE))
                {
                    return true;
                }
            }
